...
<body>
   ...
   // echo  $youEmail,  $youPass,  $youPassC, $youName, $youBirth, $youNumber;
   ...
   // var_dump(filter_var('sample@example.com', FILTER_VALIDATE_EMAIL));
    ...
    if($result){
       ...
       if( $count ==0){
           ...
           msg("회원가입을 했었네요!!!. 로그인 해주세요!!");
           exit;
       }

    }
    else{
        msg("관리자에 문의하세요. 에러발생01");
        exit;
    }
    ...
    if($result){
     ...
     msg("관리자에 문의하세요. 에러발생02");
     exit;
    }

    //회원가입
    if($isEmailCheck == true && $isNumberCheck == ture){
        $sql= "INSERT INTO myMember(youEmail, youName, youPass, youBirth, youNumber, regTime) 
        VALUES('$youEmail','$youName','$youPass','$youBirth','$youNumber','$regTime')";
         ...
         if($result){
            ...
            $_SESSION['membberID']= $connect -> insert_id;
            $_SESSION['youName']= $youName;
            //  exit;
         }else{
             msg("관리자에게 문의하세요. 에러발생03");
             exit;
         }
    }else{
        msg("다시한번 확인하고 회원가입해주세요!!");
        exit;
    }
